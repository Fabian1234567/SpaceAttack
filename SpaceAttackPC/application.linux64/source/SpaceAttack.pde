...
void draw()
{
  ...
  switch (gameMode)
  {
    ...
    case 2: //init
      ...
      {
        targetDelayCnt[i] = int(random(targetDelayMin, targetDelayMax));  // generate delay
      }
    ...
    case 3: // inGame - we do all active gaming stuff here
      ...
      for (int i=0; i<maxTarget; i++)  // all others with random
      {
        ...
        { // 04
          ...
          {
            targetActive[i] = 1;
            targetDelayCnt[i] = int(random(targetDelayMin, targetDelayMax));  // generate delay for next time
            targetX[i] = int(random(sizeX)); // define random x position 
